.data
	...
	Newline:    	.asciiz "\n"
	Prompt:     	.asciiz ": "
	Result:     	.asciiz "Phan tu xuat hien nhieu nhat la: "
...
init:
    	...
    	la 	$a1, A  		# $a1 = address of A[0]
    	move 	$s1, $s0           	# $s1 = $s0 = N
    	li 	$t1, -1 	     	# $t1 = count_max = -1 ( phần tử xuất hiện nhiều nhất) 
    	li 	$t2, 0               	# $t2 = biến lưu phần tử xuất hiện nhiều nhất
    	li 	$t3, 0               	# $t3 = count = 0
...
update:
    	bgt 	$t3, $t1, update_max	# Nếu count > count_max => update count = count_max 
    	addi 	$t0, $t0, 1		# i++
...
update_max:
    	move 	$t1, $t3	# Cập nhật count_max vào $t1
    	move 	$t2, $t4	# Cập nhật phần tử xuất hiện nhiều nhất là phần tử thứ i vào $t2
    	addi 	$t0, $t0, 1	# i++
    	addi 	$a1, $a1, 4	# Di chuyển con trỏ đến index tiếp ( là A[i + 1])
    	li 	$t3, 0		# Đặt lại count = 0
    	j 	count
