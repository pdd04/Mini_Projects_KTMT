...
main:
	...
	# Khởi tạo biến $s1 để lưu giá trị chữ số lớn nhất, ban đầu gán bằng 0
	li $s1, 0
	# Gọi hàm check để tìm chữ số lớn nhất
	j check
...
check:
	...
	addi $s0, $t2, 0
	# So sánh chữ số hiện tại với chữ số lớn nhất hiện tại
	bgt $t1, $s1, Update 
	# Nếu chữ số hiện tại không lớn hơn chữ số lớn nhất, tiếp tục kiểm tra các chữ số khác
	j check
	
Update:
	# Nếu chữ số hiện tại lớn hơn chữ số lớn nhất, cập nhật lại giá trị của chữ số lớn nhất
	addi $s1, $t1, 0
...
PrintResult:
	# In thông báo chữ số lớn nhất của N
	li $v0, 4
	la $a0, Message1
	syscall
	# In chữ số lớn nhất
