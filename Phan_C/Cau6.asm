.data
    	...
    	Message1:   	.asciiz "Ky tu chu thuong xuat hien it nhat trong xau la: "   
...
main:
    # Nhap sau:
    	...
    	la  	$a1, String	# Load địa chỉ của String để truyền vào hàm tìm ký tự chữ thường
    	li  	$a2, 1		# Khởi tạo tham số thứ 2 là 1 (đếm ký tự chữ thường)
    	addi    $fp, $sp, 0	# Khởi tạo frame pointer

    	# Gọi hàm PushStack để đếm số lượng ký tự chữ thường và lưu vào ngăn xếp
    	j   PushStack
...
# Hàm đệ quy để đếm số lượng ký tự chữ thường và lưu vào ngăn xếp
PushStack:
    	lb  	$t0, 0($a1)		# Load ký tự từ mảng String
    	beqz    $t0, FindMinFreq	# Nếu gặp ký tự null (kết thúc chuỗi), chuyển sang tìm ký tự chữ thường xuất hiện ít nhất
    	blt 	$t0, 97, continue       # Nếu ký tự không thuộc bảng mã ASCII của chữ cái hoa, tiếp tục vòng lặp
    	bgt 	$t0, 122, continue
...
# Hàm tìm ký tự chữ thường xuất hiện ít nhất trong xâu và in kết quả
FindMinFreq:
    	addi    $sp, $fp, 0	# Đặt con trỏ ngăn xếp về đầu ngăn xếp
    	li  	$t0, 0		# Khởi tạo số lần xuất hiện của ký tự chữ thường
    	li  	$t1, 99999	# Khởi tạo số lượng ký tự chữ thường có giá trị rất lớn để tìm số lượng ký tự ít nhất

MinFreq:
    	beq 	$t0, $a3, PrintResult	# Nếu đã duyệt qua hết các ký tự chữ thường, chuyển sang in kết quả
    	lb  	$t2, 0($sp)             # Load ký tự từ ngăn xếp
    	lw  	$t3, 4($sp)             # Load số lần xuất hiện của ký tự từ ngăn xếp
    	blt 	$t3, $t1, UpdateMinFreq    	# Nếu số lần xuất hiện của ký tự nhỏ hơn số lượng ký tự đã xét trước đó, cập nhật kết quả

ContinueFind:
    	addi    $sp, $sp, 8	# Di chuyển con trỏ ngăn xếp
    	addi    $t0, $t0, 1	# Tăng biến đếm
    	j   	MinFreq		# Tiếp tục vòng lặp

UpdateMinFreq:
    	addi    $s1, $t2, 0	# Lưu ký tự chữ thường xuất hiện ít nhất
    	addi    $t1, $t3, 0	# Cập nhật số lần xuất hiện của ký tự chữ thường
    	j   	ContinueFind	# Tiếp tục tìm ký tự chữ thường xuất hiện ít nhất
...
# In vị trí của các ký tự chữ thường xuất hiện nhiều ít trong xâu
    	j   	FindLocation

# Hàm tìm và in vị trí của các ký tự chữ thường xuất hiện ít nhất trong xâu
FindLocation:
    	la  	$a1, String	
...
PrintLocation:
    	...
    	bne 	$v1, $s1, PrintLocation # Nếu ký tự không trùng với ký tự chữ thường xuất hiện ít nhất, tiếp tục vòng lặp
    	li  	$v0, 1                  
